class ParseResult {
    constructor(
        ...
        this.chunks = chunks;
        this.instrumentIDMap = instrumentIDMap;
    }
};
...
export class Parser{
    constructor() {
        // MIDI音階 -> Hz
        this.frequencyTable = new Array();
        for (let i = 0; i < 128; i++) {
            this.frequencyTable[i] = 440 * (2.0 ** ((i - 69) / 12.0)) + 0.5;
        }
    }

    async parseFile(/** @type {File} */ file) {
    ...
    _parseDLS(/** @type{ArrayBuffer} */ arrayBuffer) {
        ...
        const getWvplChunk = (offset) => {
            ...
            if (subKey === 'LIST') {
                ...
                if (subSubKey === 'wave') {
                    ...
                    const wave = arrayBuffer.slice(offset + 12, offset + 8 + subSize);
